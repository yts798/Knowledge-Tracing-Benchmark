...
from utils import collate_fn_p2
...
class DKVMN_P2(KTStrategy):
    ...
    def run_model(self):
        ...
        model = DKVMN_Model(dataset.num_q, dataset.num_a, dataset.num_b, **model_config).to(device)
...
#     '''
#     def __init__(self, num_q, num_a, num_b, dim_s, size_m):
#         super().__init__()
#         self.num_q = num_q
#         self.dim_s = dim_s
#         self.size_m = size_m
...
#         self.vb_emb_layer = Embedding(self.num_b * 2, self.dim_s)
#
...
#         vb = self.vb_emb_layer(bx)
#         w = torch.softmax(torch.matmul(k, self.Mk.T), dim=-1)
#
#         # Write Process
#         e = torch.sigmoid(self.e_layer(v+va+vb))
#         a = torch.tanh(self.a_layer(v+va+vb))
#
#         for et, at, wt in zip(
...
#                 self.train()
#
#                 p, _ = self(q.long(), r.long(), a.long(), b.long())
