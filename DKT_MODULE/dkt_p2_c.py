...
from datareaderp2 import readerp2
...
class DKT_P2_C(KTStrategy):
    ...
    def run_model(self):
        ...
        dataset = readerp2(seq_len, self.data_name)
        ...
        model = DKT_Model(dataset.num_q, dataset.num_a, dataset.num_b,**model_config).to(device)
        ...
        train_loader = DataLoader(
            ...
            collate_fn=collate_fn_p2,generator=torch.Generator(device=device)
...
class DKT_Model(Module):
    ...
    '''
    def __init__(self, num_q, num_a, num_b, emb_size, hidden_size):
        super().__init__()
        self.num_q = num_q
        self.num_a = num_a
        self.num_b = num_b
        ...
        self.lstm_layer = LSTM(
            self.emb_size+num_a+num_b, self.hidden_size, batch_first=True
        )
        self.c_integration = CatEmbedding(num_a, num_b, emb_size)
        # self.out_layer = Linear(self.hidden_size+num_a+num_b, self.num_q)
        self.out_layer = Linear(self.hidden_size, self.num_q)
        self.dropout_layer = Dropout()


    def forward(self, q, r, a, b):
        '''
            Args:
        ...
        theta_in = self.c_integration(x_emb, a.long(), b.long())
        h, _ = self.lstm_layer(theta_in)
        # theta_out = self.c_integration(h, a.long(), b.long())
    ...
    def train_model(
        ...
        for i in range(1, num_epochs + 1):
            ...
            for data in train_loader:
                q, r, qshft, rshft, a, ashft, b, bshft, m = data

                self.train()

                y = self(q.long(), r.long(), a.long(), b.long())
                # print(y.shape)
