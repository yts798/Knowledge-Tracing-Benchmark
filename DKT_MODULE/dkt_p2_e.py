...
class DKT_Model(Module):
    ...
    def __init__(self, num_q, num_a, num_b, emb_size, hidden_size):
        ...
        self.interaction_emb_a = Embedding(self.num_a * 2, self.emb_size)
        self.interaction_emb_b = Embedding(self.num_b * 2, self.emb_size)
    ...
    def forward(self, q, r, a, b):
        ...
        ax_emb = self.interaction_emb_a(ax)
        bx_emb = self.interaction_emb_b(bx)
        h, _ = self.lstm_layer(x_emb+ax_emb+bx_emb)
        # print(h.shape)
        y = self.out_layer(h)
